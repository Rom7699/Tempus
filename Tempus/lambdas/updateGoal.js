...
exports.handler = async (event) => {
  ...
  try {
    ...
    const allowedFields = [
      ...
      'current_cycle_start', 'current_cycle_end', 'cycles_completed', 'total_cycles'
    ];
    ...
    const needsRecalculation = fields.goal_start_date || fields.goal_cycle_duration || fields.goal_type;
    ...
    if (needsRecalculation) {
      ...
      // Recalculate goal_end_date if has duration (all goals are cycling)
      if (updatedGoal.goal_cycle_duration && updatedGoal.goal_cycle_duration !== 'forever') {
      ...
      // Recalculate current cycle dates (all goals are cycling)
      const startDate = new Date(updatedGoal.goal_start_date);
