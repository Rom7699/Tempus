...
const DisplayTaskModal: React.FC<DisplayTaskModalProps> = ({
    ...
    const getPriorityInfo = (priority: number | undefined) => {
        ...
        switch (priority) {
            case 1: return { text: 'Low', color: '#34A853' };
            case 2: return { text: 'Medium', color: '#FBBC05' };
            case 3: return { text: 'High', color: '#EA4335' };
            default: return { text: 'None', color: '#888' };
        }
    };
    // Confirm delete
    const confirmDelete = () => {
                        ...
                        <ScrollView style={styles.modalContent}>
                                ...
                                {localTask.is_event === true && (
                                    <TouchableOpacity
                                        style={[
                                            ...
                                            localTask.is_completed ? styles.completedStatus : styles.pendingStatus
                                        ]}
                                        onPress={handleToggle}
                                    >
                                        <Ionicons
                                            name={localTask.is_completed ? "checkmark-circle" : "ellipse-outline"}
                                            size={18}
                                            color={localTask.is_completed ? "#34A853" : "#666"}
                                        />
                                        <Text style={localTask.is_completed ? styles.completedText : styles.pendingText}>
                            ...
                            {localTask.task_description && (
                                <View style={styles.section}>
                                    ...
                                    <Text style={styles.sectionText}>{localTask.task_description}</Text>
                                </View>
                            )}
                            ...
                            <View style={styles.section}>
                                ...
                                <View style={styles.infoRow}>
                                    ...
                                    <View style={styles.infoSeparator} />
                                    <View style={styles.infoItem}>
                                        <Ionicons name="battery-charging-outline" size={20} color="#5D87FF" />
                                        <Text style={styles.infoLabel}>Energy</Text>
                                        <Text style={styles.infoValue}>{localTask.task_energy_level || 'N/A'}</Text>
                                    </View>
                                </View>
                            </View>
                            ...
                            {localTask.task_location && (
                                <View style={styles.section}>
                                    ...
                                    <View style={styles.locationContainer}>
                                        <Ionicons name="location-outline" size={20} color="#5D87FF" />
                                        <Text style={styles.locationText}>{localTask.task_location}</Text>
                                    </View>
                                </View>
                            )}

                            {/* Attendees if available */}
                            {localTask.task_attendees && localTask.task_attendees.length > 0 && (
                                <View style={styles.section}>
                                    <Text style={styles.sectionTitle}>Attendees</Text>
                                    {localTask.task_attendees.map((attendee, index) => (
                                        <View key={index} style={styles.attendeeItem}>
                                            <Ionicons name="person-outline" size={18} color="#5D87FF" />
                            ...
                            <View style={styles.section}>
                                <View style={styles.reminderRow}>
                                    <Ionicons
                                        name={localTask.task_reminder ? "notifications" : "notifications-outline"}
                                        size={20}
                                        color="#5D87FF"
                                    />
